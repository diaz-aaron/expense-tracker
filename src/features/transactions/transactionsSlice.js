...
const transactionsSlice = createSlice({
  ...
  reducers: {
    ...
    deleteTransaction: (state, action) => {
      const transactionToDelete = state[action.payload.category].findIndex((transaction) => transaction.id === action.payload.id);
      state[action.payload.category].splice(transactionToDelete, 1);
    } 
  }
})
